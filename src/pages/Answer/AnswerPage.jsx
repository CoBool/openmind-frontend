...
function AnswerPage() {
  ...
  const handleUpdateAnswer = async question => {
    ...
    try {
      ...
      const nextAnswer = updated?.answer ??
        updated ?? { ...question.answer, content };
      setQuestions(prev =>
        prev.map(q => (q.id === question.id ? { ...q, answer: nextAnswer } : q))
      );
      setEditingQuestionId(null);
    ...
    <div className={styles.page}>
      ...
      <main className={styles.list}>
        {questions.map(question => {
          ...
          const draft =
            draftByQuestionId[question.id] ??
            (hasAnswer ? question.answer.content : '');
            ...
            <Card key={question.id}>
              <div className={styles.cardTopRow}>
                <span
                  className={[
                    styles.badge,
                    hasAnswer ? styles.badgeDone : styles.badgeTodo,
                  ].join(' ')}
                >
                  {hasAnswer ? '답변 완료' : '미답변'}
                </span>
              ...
              <div className={styles.questionBlock}>
                <div className={styles.questionMeta}>
                  질문 · {question?.createdAt?.slice(0, 10) ?? ''}
                </div>
